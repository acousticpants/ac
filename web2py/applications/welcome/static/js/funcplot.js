...
Muster[ 0] = "   #"; Muster[ 1] = "    "; Muster[ 2] = " #  "; Muster[ 3] = "    ";
Muster[ 4] = "  # "; Muster[ 5] = "#   "; Muster[ 6] = "   #"; Muster[ 7] = " #  ";
Muster[ 8] = "#   "; Muster[ 9] = "  ##"; Muster[10] = "##  "; Muster[11] = "   #";
Muster[12] = "  ##"; Muster[13] = "##  "; Muster[14] = "  ##"; Muster[15] = "##  ";
Muster[16] = "### "; Muster[17] = " # #"; Muster[18] = "# # "; Muster[19] = " ###";
Muster[20] = " ###"; Muster[21] = "## #"; Muster[22] = " ###"; Muster[23] = "## #";
Muster[24] = "####"; Muster[25] = "## #"; Muster[26] = "####"; Muster[27] = "# ##";
Muster[28] = "####"; Muster[29] = "####"; Muster[30] = "####"; Muster[31] = "####";
...
function Tokenize(e)
{
	...
	while(tmp = r.exec(e))
	{
		if(Implied(previousToken) && Implied(tmp[0].toString()))
			result.push('*');
...
function ToPostfix(infix)
{
	...
	while(token = infix.shift())
	{
		...
		else if(Precedence[token])
		{
			...
			if(token == "," && !IsValidEnd(previousToken))
				postfix.push("Unexpected: " + previousToken);
			if(token != ",")
				stack.push(token);
		}
		else if(token == ")")
		{
			if(!IsValidEnd(previousToken))
				postfix.push("Unexpected: " + previousToken);
			while(stack.length && stack[stack.length - 1] != "(")
				postfix.push(stack.pop());
			if(stack.length == 0)
				postfix.push("Unmatched: )");
			stack.pop(); // discard the "("
		}
		else if(/^[-+]?(\d+\.?|\d*\.\d+)(e[-+]?\d+)?$/.test(token))
			postfix.push(token);
		else
			postfix.push("Unknown: " + token);
		previousToken = token;
	}
	if(!IsValidEnd(previousToken))
		postfix.push("Unexpected: " + previousToken);
...
function SyntaxCheck(postfix)
{
	...
	{
		if(/^Un(matched|known|expected).*/.test(token = postfix[i]))
			return token;
		if(Expects[token])
		{
			if(stack < Expects[token])
				return "Invalid use of: " + token;
			stack -= Expects[token];
		}
	}
...
function LineDirect(x1,y1,x2,y2)
{
	...
	x1 = parseInt(x1);
	y1 = parseInt(y1);
	if(x1 < 0 || x1 >= PicW || y1 < 0 || y1 >= PicH)
		i |= 1;

	x2 = parseInt(x2);
	y2 = parseInt(y2);
	if(x2 < 0 || x2 >= PicW || y2 < 0 || y2 >= PicH)
		i |= 2;
...
function asin(x) { return FromRad(Math.asin(x)); }
function acos(x) { return FromRad(Math.acos(x)); }
function atan(x) { return FromRad(Math.atan(x)); }
function asec(x) { return FromRad(Math.acos(1 / x)); }
function acsc(x) { return FromRad(Math.asin(1 / x)); }
function acot(x) { return FromRad(Math.atan(1 / x)); }
...
function atanh(x) { return Math.log((1 + x) / (1 - x)) * 0.5; }
...
function logb(y,x) { return Math.log(x) / Math.log(y); }
function pow(y,x) { return Math.pow(x,y); }
function Pow10(x) { return Math.pow(10,x); }
function sq(x) { return x * x; }
function sqrt(x) { return Math.sqrt(x); }
function abs(x) { return x >=0 ? x : -x; }
function sgn(x) { return x == 0 ? 0 : (x > 0 ? 1 : -1); }
function chs(x) { return -x; }
function neg(x) { return -x; }
function floor(x) { return Math.floor(x); }
function ceil(x) { return Math.ceil(x); }
function ip(x) { return parseInt(x); }
function fp(x) { return abs(x) - ip(abs(x)); }
function inv(x) { return 1 / x; }
function Idiv(y,x) { return parseInt(x / y); }
function mod(y,x) { return x % y; }
function and(y,x) { return x & y; }
function or(y,x) { return x | y; }
function xor(y,x) { return x ^ y; }
function not(x) { return ~x; }
function Lt(y,x) { return x < y ? 1 : 0; }
function Le(y,x) { return x <= y ? 1 : 0; }
function Eq(y,x) { return x == y ? 1 : 0; }
function Ne(y,x) { return x != y ? 1 : 0; }
function Ge(y,x) { return x >= y ? 1 : 0; }
function Gt(y,x) { return x > y ? 1 : 0; }
function min(y,x) { return Math.min(x,y); }
function max(y,x) { return Math.max(x,y); }
function Pc(y,x) { return x * y / 100; }
function Pcch(y,x) { return 100 * y / x; }
...
function comb(y,x) { return Math.floor(0.5 + Math.exp(GammaLn(x + 1.0) - GammaLn(y + 1.0) - GammaLn(x - y + 1.0))); }
function perm(y,x) { return Math.floor(0.5 + Math.exp(GammaLn(x + 1.0) - GammaLn(x - y + 1.0))); }
function gcd(a,b)
{
	if(a == 0)
		return (b || 1);
...
function Evaluate(postfix,x,y)
{
	...
	for(var i = 0; i < postfix.length; i++)
	{
		...
		else if(window[Alias[token] || token])
			stack.push(window[Alias[token] || token](stack.pop(),window[Alias[token] || token].length > 1 ? stack.pop() : 0));
		else
			stack.push(token);
...
function DoPlot2D(postfix,fnType)
{
	...
	if(fnType == 1)
	{
		...
		for(i = 0; i < PicW; i++,x += Dx)
			if(!isNaN(y = Evaluate(postfix,x,0)))
				break;
	...
	else
	{
		...
		for(i = 0; i < PicH; i++,y += Dy)
			if(!isNaN(x = Evaluate(postfix,0,y)))
				break;
...
function DoPlot3D(postfix)
{
	...
	for(i = 0; i < samples; i++)
	{
		...
		for(j = 0; j < samples; j++)
		{
			...
			if(!isFinite(z = Evaluate(postfix,x,y)))
				z = 0;
			graph.push(z);
			zi = Math.min(zi,z);
			za = Math.max(za,z);
...
function Remove()
{
	...
	if(FM.selectedIndex >= --FM.length && FM.length)
		FM.options[FM.length - 1].selected = true;
}
